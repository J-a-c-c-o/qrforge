fn main() {
    ...
    let combined_data = build_combined_data(mode_indicator, size, data, data_codewords);

    //print combined data in a nice format so binary can be easily read each 8 bits a space
